...
class MainApp(QtWidgets.QMainWindow):
    ...
    def init_ui(self):
        ...
        self.setWindowTitle("Camera and Pose Estimation")
        ...
        # Pose estimation display area pose estimation 
        self.pose_label = QtWidgets.QLabel("Pose Estimation")
        ...
        self.load_pose_btn = QtWidgets.QPushButton("Load Pose Estimation", clicked=self.load_pose_image)
        button_layout.addWidget(self.load_pose_btn)
        layout.addLayout(button_layout)

        self.camera_thread = MyThread()
        self.camera_thread.frame_signal.connect(self.set_camera_image)
    ...
    def load_pose_image(self):
        # Assuming your pose estimation program has generated an image file
        pose_image_path = "pose_result.png"  # Replace with your image path
        pose_image = QImage(pose_image_path)
        if pose_image.isNull():
            QtWidgets.QMessageBox.critical(self, "Error", f"Failed to load image: {pose_image_path}")
        else:
            self.pose_label.setPixmap(QPixmap.fromImage(pose_image))
...
def parse_args():
    ...
    parser.add_argument(
        '--show-interval', type=int, default=0.0001, help='Sleep seconds per frame')
    parser.add_argument(
        '--thickness',
        type=int,
        default=3,
...
#def pose_estimation_inference(frame, det_config, det_checkpoint, pose_config, pose_checkpoint, device='cuda:0'):
def pose_estimation_inference(frame, detector, pose_estimator):

    """
    Use mmpose to perform pose estimation and return a visualization of the resulting image.
    
    Args:
        frame (np.ndarray): Input image frames.
        det_config (str): Detection model configuration file path.
        det_checkpoint (str): Detection model weight file path.
        pose_config (str): Pose estimation configuration file path. 
        pose_checkpoint (str): Pose estimation weight file path. 
        device (str): Inference device, default is 'cuda:0'.

    Returns:
        np.ndarray: Visualized image frames.
    """    
    ...
    bboxes = bboxes[pred_instances.scores > 0.5]  # Threshold filtering
    ...
    visualized_frame = visualizer.get_image()
