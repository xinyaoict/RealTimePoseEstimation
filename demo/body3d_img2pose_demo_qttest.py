...
def process_one_image(args, detector, frame: np.ndarray, frame_idx: int,
    ...
    if pred_3d_instances is not None:
        ...
        links = pose_estimator.dataset_meta.get('skeleton_links', [])

    return pose_est_results, pose_est_results_list, keypoints, links, next_id
    #return pose_est_results, pose_est_results_list, pred_3d_instances, next_id

def process_frames(video_source, detector, pose_estimator, visualizer, args):
    """
    Generator to process frames from the video or webcam.
    """
    next_id = 0
    pose_est_results = []
    pose_est_results_list = []

    video = cv2.VideoCapture(0 if video_source == "webcam" else video_source)
    while video.isOpened():
        success, frame = video.read()
        if not success:
            break

        # 调用 process_one_image 获取关键点和链接数据
        pose_est_results, pose_est_results_list, keypoints, links, next_id = process_one_image(
            args=args,
            detector=detector,
            frame=frame,
            frame_idx=0,
        ...
        yield keypoints, links  # 返回关键点和骨架
