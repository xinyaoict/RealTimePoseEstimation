...
def process_one_image(args, detector, frame: np.ndarray, frame_idx: int,
    ...
    # sort keypoints by z-axis
    # 假设 keypoints 的形状是 (batch_size, num_keypoints, 3)
#    keypoints = np.take_along_axis(
#        keypoints, keypoints[:, :, 2].argsort(axis=1)[..., None], axis=1
#    )
    
    #keypoints = keypoints[keypoints[:, :, 2].argsort(axis=1)]

    #print("Keypoints:", keypoints)
    #print("Keypoints shape:", keypoints.shape)
    
    for idx, pose_est_result in enumerate(pose_est_results):
        # 获取关键点坐标 (假设右肩、右肘、右手腕为第6、8、10个关键点)
        right_shoulder = keypoints[idx, 6]
        right_elbow = keypoints[idx, 8]
        right_wrist = keypoints[idx, 10]
        
        # 计算肩-肘-手腕的夹角
        elbow_angle = calculate_angle(right_shoulder, right_elbow, right_wrist)
        
        # 将角度信息存储在 pose_est_result 中
        pose_est_result.pred_instances.elbow_angle = np.array([elbow_angle])
        #pose_est_result.pred_instances.elbow_angle = elbow_angle
    print("Elbow angle:", pose_est_results[0].pred_instances.elbow_angle)
    #print(pred_3d_data_samples)
    #print(pred_3d_instances.shape)
    # Visualization
...
def calculate_angle(joint_a, joint_b, joint_c):
    # 计算向量 AB 和 BC
    vector_ab = joint_a - joint_b
    vector_bc = joint_c - joint_b
    
    # 计算向量夹角
    cosine_angle = dot(vector_ab, vector_bc) / (norm(vector_ab) * norm(vector_bc))
    angle = arccos(cosine_angle)  # 计算弧度
    return degrees(angle)  # 返回角度
